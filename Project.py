...
selected_box = st.sidebar.selectbox(
    ...
    ('Start', 'Table Preview', 'Column Explaination', 'How to Use 3D Scatter Plot?', 'Analysis 1', 'Analysis 2')
    )
...
if selected_box == 'Analysis 1':
    st.title('Analysis 1')
    plot1 = px.scatter_3d(happy, x = 'Ladder score', y = 'Logged GDP per capita' , z = 'Social support')
    st.plotly_chart(plot1)
    st.markdown('Sub-Saharan Africa is in the lower area, poor')
...
if selected_box == 'Analysis 2':
    ...
    if st.button('Show analysis'):
        st.markdown('When the GDP goes up')
st.markdown('GDP up, Ladder score up')
